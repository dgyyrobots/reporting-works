<template>
  ...
  <content-wrap>
    <el-form ref="queryFormRef" class="-mb-15px" :inline="true" label-width="68px" :model="queryParams">
      ...
      <el-form-item label="模板编号" prop="code">
        <el-input v-model="queryParams.code" class="!w-240px" clearable placeholder="请输入模版编码" @keyup.enter="handleQuery" />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-hasPermi="['system:notify-template:create']" plain type="primary" @click="openForm('create')">
  ...
  <content-wrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="210">
        <template #default="scope">
          <el-button v-hasPermi="['system:notify-template:update']" link type="primary" @click="openForm('update', scope.row.id)">
            修改
          </el-button>
          <el-button v-hasPermi="['system:notify-template:send-notify']" link type="primary" @click="openSendForm(scope.row)">
            测试
          </el-button>
          <el-button v-hasPermi="['system:notify-template:delete']" link type="danger" @click="handleDelete(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
  ...
  <!-- 表单弹窗：添加/修改 -->
  <notify-template-form ref="formRef" @success="getList" />
  <!-- 表单弹窗：测试发送 -->
  <notify-template-send-form ref="sendFormRef" />
</template>
