<template>
  <Dialog v-model="dialogVisible" title="修改任务规则" width="600">
    <el-form ref="formRef" label-width="80px" :model="formData" :rules="formRules">
      ...
      <el-form-item v-if="formData.type === 20 || formData.type === 21" label="指定部门" prop="deptIds" span="24">
        <el-tree-select
          ref="treeRef"
      ...
      <el-form-item v-if="formData.type === 30 || formData.type === 31 || formData.type === 32" label="指定用户" prop="userIds" span="24">
      ...
      <el-form-item v-if="formData.type === 50" label="指定脚本" prop="scripts">
        <el-select v-model="formData.scripts" clearable multiple style="width: 100%">
          <el-option v-for="dict in taskAssignScriptDictDatas" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
    </el-form>
    <!-- 操作按钮 -->
