<template>
  <ContentWrap>
    ...
    <el-form ref="queryFormRef" class="-mb-15px" :inline="true" label-width="68px" :model="queryParams">
      <el-form-item label="流程标识" prop="key">
        <el-input v-model="queryParams.key" class="!w-240px" clearable placeholder="请输入流程标识" @keyup.enter="handleQuery" />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-hasPermi="['bpm:model:create']" plain type="primary" @click="openForm('create')">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="表单信息" prop="formType" width="200">
        <template #default="scope">
          <el-button v-if="scope.row.formType === 10" link type="primary" @click="handleFormDetail(scope.row)">
            <span>{{ scope.row.formName }}</span>
          </el-button>
          <el-button v-else-if="scope.row.formType === 20" link type="primary" @click="handleFormDetail(scope.row)">
      ...
      <el-table-column align="center" label="最新部署的流程定义">
        <el-table-column align="center" label="流程版本" prop="processDefinition.version" width="100">
          <template #default="scope">
            <el-tag v-if="scope.row.processDefinition">v{{ scope.row.processDefinition.version }}</el-tag>
            <el-tag v-else type="warning">未部署</el-tag>
          </template>
        </el-table-column>
        <el-table-column align="center" label="激活状态" prop="processDefinition.version" width="85">
          <template #default="scope">
            <el-switch
      ...
      <el-table-column align="center" fixed="right" label="操作" width="240">
        <template #default="scope">
          <el-button v-hasPermi="['bpm:model:update']" link type="primary" @click="openForm('update', scope.row.id)">修改流程</el-button>
          <el-button v-hasPermi="['bpm:model:update']" link type="primary" @click="handleDesign(scope.row)">设计流程</el-button>
          <el-button v-hasPermi="['bpm:task-assign-rule:query']" link type="primary" @click="handleAssignRule(scope.row)">
            分配规则
          </el-button>
          <el-button v-hasPermi="['bpm:model:deploy']" link type="primary" @click="handleDeploy(scope.row)">发布流程</el-button>
          <el-button v-hasPermi="['bpm:process-definition:query']" link type="primary" @click="handleDefinitionList(scope.row)">
            流程定义
          </el-button>
          <el-button v-hasPermi="['bpm:model:delete']" link type="danger" @click="handleDelete(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
  ...
  <Dialog v-model="bpmnDetailVisible" title="流程图" width="800">
    <MyProcessViewer key="designer" v-model="bpmnXML" v-bind="bpmnControlForm" :prefix="bpmnControlForm.prefix" :value="bpmnXML as any" />
  </Dialog>
</template>
