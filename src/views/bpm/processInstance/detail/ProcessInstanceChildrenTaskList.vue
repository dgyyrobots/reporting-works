<template>
  <el-drawer v-model="drawerVisible" size="70%" title="子任务">
    <!-- 当前任务 -->
    <template #header>
      <h4>【{{ baseTask.name }} 】审批人：{{ baseTask.assigneeUser?.nickname }}</h4>
      <el-button v-if="isSubSignButtonVisible(baseTask)" plain style="margin-left: 5px" type="danger" @click="handleSubSign(baseTask)">
        <Icon icon="ep:remove" />
        减签
      </el-button>
    </template>
    <!-- 子任务列表 -->
    <el-table border :data="baseTask.children" row-key="id" style="width: 100%">
      <el-table-column label="审批人" prop="assigneeUser.nickname" />
      <el-table-column label="所在部门" prop="assigneeUser.deptName" />
      ...
      <el-table-column label="操作" prop="operation">
        <template #default="scope">
          <el-button v-if="isSubSignButtonVisible(scope.row)" plain type="danger" @click="handleSubSign(scope.row)">
            <Icon icon="ep:remove" />
            减签
          </el-button>
        </template>
