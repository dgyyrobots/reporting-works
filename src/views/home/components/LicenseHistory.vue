<template>
  <Card class="LicenseHistory" content-padding="6px 0" title="生产版号" :showEmpty="!loading && tableData.length === 0">
    <template #titleRight>
      ...
      <table v-else-if="tableData.length > 0">
        ...
        <tbody>
          <tr v-for="(item, index) in tableData" :key="index">
            ...
            <td >
          <span  :class="{ 
            warning: getStatusText(item.status_id) === '未采集',
            collected: getStatusText(item.status_id) === '已采集'
          }">{{ getStatusText(item.status_id) }}</span>
        
        </td>
            <td>{{ toInteger(item.pass_uqty) }}</td>
...
// 表头定义
const tableHeaders = ['版号', '采集数量', '采集时间', '状态', '过版数量', '不合品数量']
...
const fetchData = async () => {
  ...
  try {
    ...
    const params = {
      filter: [{"val":[{"name":"device_id","val":props.currentDevice.id,"action":"="},{"name":"jobbill_id","val":jobbill_id.value,"action":"="}],"relation":"AND"}],
      filter_detail: {},
      other_params: {"structure":"entry1"},
      keyword_is_detail: '0',
      sum_col: ["collection_uqty","collection_qty","pass_uqty","pass_qty","ok_uqty","ok_qty","no_ok_uqty","no_okqty","scrap_uqty","scrap_qty","loss_uqty","loss_qty"],
      sum_col_type: '1',
