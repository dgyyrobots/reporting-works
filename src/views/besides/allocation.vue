<template>
  <div class="app-container">
    <!-- 搜索工作栏 -->
    <el-form v-show="showSearch" ref="queryForm" :inline="true" label-width="100px" :model="queryParams" :size="currentSize">
      <el-form-item label="调拨单号" prop="allocatedCode">
        <el-input v-model="queryParams.allocatedCode" clearable placeholder="请输入调拨单编号" @keyup.enter="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="仓库名称" prop="warehouseName">
        <el-input v-model="queryParams.warehouseName" clearable placeholder="请输入仓库名称" @keyup.enter="handleQuery" />
      </el-form-item>
    ...
    <el-row class="mb8" :gutter="10">
      <el-col :span="1.5">
        <el-button v-hasPermi="['wms:allocated-header:create']" :icon="Plus" :size="currentSize" type="primary" @click="handleAdd">
          新增
        </el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button
          :icon="Download"
          v-hasPermi="['wms:allocated-header:export']"
      ...
      <el-col :span="1.5">
        <el-button
          :disabled="single"
          v-hasPermi="['wms:allocated-header:finsh']"
          :icon="Edit"
          plain
          :size="currentSize"
          type="success"
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['wms:allocated-header:createIssue']"
          :disabled="single"
          :icon="Edit"
          plain
          :size="currentSize"
    ...
    <div ref="tableContainerRef" class="table-container">
      ...
      >
        <el-table-column v-if="!isExecuteDetail" align="center" type="selection" width="55" />
        <el-table-column align="center" label="调拨单编号" prop="allocatedCode" />
        <el-table-column align="center" label="调拨单名称" prop="allocatedName" />
        <el-table-column align="center" label="生产工单" prop="workorderCode" />
        <el-table-column align="center" label="调拨日期" prop="allocatedDate">
          <template #default="scope">
            <span>{{ formatDate(scope.row.allocatedDate, 'YYYY-MM-DD') }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="单据状态" prop="status">
          <template #default="scope">
        ...
        <el-table-column align="center" class-name="small-padding fixed-width" fixed="right" label="操作">
          <template #default="scope">
            <el-button
              ...
              v-hasPermi="['wms:allocated-header:allocated']"
              :size="currentSize"
              type="text"
            ...
            <el-button v-hasPermi="['wms:allocated-header:update']" :size="currentSize" type="text" @click="handleUpdate(scope.row)">
              修改
            </el-button>
            <el-button v-hasPermi="['wms:allocated-header:delete']" :size="currentSize" type="text" @click="handleDelete(scope.row)">
              删除
            </el-button>
            <el-button
              v-hasPermi="['wms:allocated-header:allocated']"
              :size="currentSize"
              type="text"
              @click="handleExecuteDetail(scope.row)"
            >
              详情
            </el-button>
          </template>
    ...
    <!-- 对话框(添加 / 修改) -->
    <el-dialog v-model="open" v-dialogDrag append-to-body :title="title" width="90%">
      <el-form ref="formRef" :model="form" :rules="rules" :size="currentSize">
        <el-row>
          <el-col :span="8">
            <el-form-item label="调拨单编号" prop="allocatedCode">
              <el-input v-model="form.allocatedCode" :disabled="isEdit" placeholder="请输入调拨单编号" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="调拨单名称" prop="allocatedName">
              <el-input v-model="form.allocatedName" disabled placeholder="请输入调拨单名称" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="调拨日期" prop="allocatedDate">
              <el-date-picker
                v-model="form.allocatedDate"
                clearable
                placeholder="选择调拨日期"
                type="date"
                @change="handAllocatedDateChangeForm"
              />
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="单据状态" prop="status">
              <el-select v-model="form.status" disabled>
                <el-option v-for="dict in DICT_TYPE.MES_ALLOCATED_STATUS" :key="dict.value" :label="dict.label" :value="dict.value" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label-width="80">
              <el-switch
                v-if="optType != 'view' && form.status == 'PREPARE'"
                v-model="form.bindWorkorder"
                active-color="#13ce66"
                inactive-text="绑定工单"
                style="margin-left: 10%"
                @change="handleBindWorkorderChange"
              />
            </el-form-item>
          </el-col>

          <el-col v-if="form && form.bindWorkorder" :span="8">
            ...
            <WorkorderSelect ref="woSelectRef" @on-selected="onWorkorderSelected" />
          </el-col>
          ...
          <el-col v-if="form && form.bindWorkorder" :span="8">
            ...
            <ProtaskSelect ref="taskSelectRef" :workorder-id="form.workorderId" @on-selected="onTaskSelected" />
          </el-col>
        </el-row>
      ...
      <div v-if="form && form.bindWorkorder">
        ...
        <el-card class="box-card" shadow="always">
          ...
          >
            ...
            <el-table-column align="center" label="物料编码" prop="itemCode" />
            <el-table-column align="center" label="物料名称" prop="itemName" :show-overflow-tooltip="true" />
            <el-table-column align="center" label="需求数量" prop="quantityAllocated" width="210" />
            <el-table-column align="center" label="单位" prop="unitOfMeasure" width="80" />
          </el-table>
        </el-card>

        <el-divider content-position="center">调拨信息</el-divider>
        <el-card class="box-card" shadow="always">
          <el-table v-loading="bomLoadding" :data="allocatedList" max-height="300" row-key="itemCode" :size="currentSize">
            <el-table-column align="center" label="物料编码" prop="itemCode" :show-overflow-tooltip="true" />
            <el-table-column align="center" label="物料名称" prop="itemName" :show-overflow-tooltip="true" />
            <el-table-column align="center" label="调拨数量" prop="quantityAllocated" width="220">
              <template #default="scope">
                <el-input
    ...
    <el-dialog
      ...
      :title="isExecuteDetail ? '详情' : '执行出库'"
      width="92%"
    >
      <el-form ref="executeFormRef" label-width="100px" :model="executeForm" :size="currentSize">
        <el-row>
          <el-col :span="8">
            <el-form-item label="调拨单编号" prop="allocatedCode">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="调拨日期" prop="allocatedDate">
              <el-date-picker
                v-model="executeForm.allocatedDate"
                clearable
                disabled
                placeholder="选择调拨日期"
                type="date"
                @change="handAllocatedDateChangeExecuteForm"
              />
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row v-if="executeForm.bindWorkorder">
          <el-col :span="8">
            <el-form-item label="生产工单" prop="workorderCode">
              <el-input v-model="executeForm.workorderCode" disabled placeholder="请选择生产工单">
                <template #append>
                  <el-button disabled :icon="Search" @click="handleWorkorderSelect" />
                </template>
              </el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8">
            <el-form-item label="生产任务" prop="taskCode">
              <el-input v-model="executeForm.taskCode" disabled placeholder="请选择生产任务">
                <template #append>
                  <el-button disabled :icon="Search" @click="handleTaskSelect" />
                </template>
              </el-input>
            </el-form-item>
            <ProtaskSelect ref="taskSelectRef" :workorder-id="executeForm.workorderId" @on-selected="onExecuteTaskSelected" />
          </el-col>

          <el-col :span="8">
            <el-form-item label="客户编号">
              <el-input v-model="executeForm.clientCode" disabled placeholder="请选择生产工单" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="备注" prop="remark">
              <el-input v-model="executeForm.remark" disabled placeholder="请输入内容" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-table
          v-if="executeForm.bindWorkorder"
          v-loading="bomLoadding"
          :data="bomList"
          max-height="200"
          :size="currentSize"
        ...
        >
          <el-table-column align="center" label="物料编码" prop="itemCode" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="物料名称" prop="itemName" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="需求数量" prop="quantityAllocated" />
          <el-table-column align="center" label="单位" prop="unitOfMeasure" width="80" />
        </el-table>

        <el-divider content-position="center">调拨信息</el-divider>

        <el-row v-if="!isExecuteDetail" :gutter="10">
          <el-col :span="8">
            <el-form-item label="单据信息" prop="purchaseId">
              <el-input v-model="purchaseId" placeholder="请输入" readonly />
              <input
                ref="scannerInput"
                v-model="scanData"
                autofocus
                inputmode="none"
                placeholder="请输入"
                style="position: absolute; opacity: 0; width: 1; height: 1; z-index: -1; -webkit-user-select: none"
              />
          ...
          <el-col :span="4">
            ...
            <el-button round type="primary" @click="handResetPurchaseId">重置</el-button>
          </el-col>
          <el-col :span="2.5">
            <el-button :icon="Plus" :size="currentSize" style="margin-right: 10px" type="primary" @click="allocatedHandleAdd">
              新增
            </el-button>
          </el-col>
          ...
          <el-col :span="2.5">
            <el-button
              v-hasPermi="['wms:allocated-header:batchAdd']"
              :icon="Plus"
              :size="currentSize"
              style="margin-right: 10px"
              type="primary"
              @click="allocatedHandleBatchAdd"
            >
              批量新增
            </el-button>
          </el-col>
          <StockSelect ref="stockSelectRef" @on-selected="onStockSelected" />

          <el-col :span="2.5">
            <el-button :disabled="allocatedSingle" :icon="Delete" :size="currentSize" type="danger" @click="allocatedHandleDelete">
              删除
            </el-button>
          </el-col>
        </el-row>
        <el-table v-loading="loading" :data="allocatedList" :size="currentSize" @selection-change="allocatedHandleSelectionChange">
          <el-table-column v-if="!isExecuteDetail" align="center" type="selection" width="55" />
          ...
          <el-table-column align="center" label="物料编码" prop="itemCode" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="物料名称" prop="itemName" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="调拨数量" prop="quantityAllocated" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="单位" prop="unitOfMeasure" width="70" />
          <el-table-column align="center" label="批次号" prop="batchCode" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="调拨标识" prop="allocatedFlag" :show-overflow-tooltip="true" width="90" />
          <el-table-column align="center" label="仓库名称" prop="warehouseName" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="库区名称" prop="locationName" :show-overflow-tooltip="true" />
          <el-table-column align="center" label="库位名称" prop="areaName" :show-overflow-tooltip="true" />
        </el-table>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button :size="currentSize" @click="cancel">取 消</el-button>
          <el-button v-if="!isExecuteDetail" :size="currentSize" type="primary" @click="executeAllocated">确 定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
import {
  createAllocatedHeader,
  updateAllocatedHeader,
  deleteAllocatedHeader,
  getAllocatedHeader,
  getAllocatedHeaderPage,
  exportAllocatedHeaderExcel,
  initBomInfo,
  execute,
  finshAllocatedHeader,
  updateAllocatedLine,
  createIssue,
} from '@/api/wms/allocatedHeader'
...
const handleUpdate = (row) => {
  ...
  getAllocatedHeader(id).then((response) => {
    ...
    // 设置领料仓库信息
    warehouseInfo.value = [response.warehouseId, response.locationId, response.areaId]
...
const submitForm = () => {
  formRef.value.validate((valid) => {
    ...
    form.value.bomList = allocatedList.value.map((item) => ({
      ...item,
      quantityAllocated: parseFloat(item.quantityAllocated),
    }))
...
const handleDelete = (row) => {
  const id = row.id
  ElMessageBox.confirm(`是否确认删除调拨单头编号为"${id}"的数据项?`)
    .then(() => {
...
const handleExport = () => {
  ...
  ElMessageBox.confirm('是否确认导出所有调拨单头数据项?')
    .then(() => {
...
const handleFinsh = (row) => {
  ...
  loading.value = true
  // 完成
  finshAllocatedHeader(allocatedId)
    .then((response) => {
...
const handleIssue = () => {
  // 确认执行吗?
  ElMessageBox.confirm('确认生成领料单吗?')
    .then(() => {
      loading.value = true
      const allocatedId = ids.value
      createIssue(allocatedId)
        .then(() => {
          loading.value = false
          getList()
