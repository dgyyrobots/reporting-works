<template>
    ...
    <el-col :span="6">
      <ContentWrap class="h-[110px] pb-0!">
        <div class="flex items-center">
          ...
          >
            <Icon icon="fa:user-times" :size="23" />
          </div>
          <div class="ml-[20px]">
            <div class="mb-8px text-14px text-gray-400">参与人数(个)</div>
            <CountTo class="text-20px" :duration="2600" :end-val="recordSummary.userCount" :start-val="0" />
          </div>
        </div>
      </ContentWrap>
    </el-col>
    <el-col :span="6">
      <ContentWrap class="h-[110px]">
        <div class="flex items-center">
          ...
          >
            <Icon icon="fa:user-plus" :size="23" />
          </div>
          <div class="ml-[20px]">
            <div class="mb-8px text-14px text-gray-400">成团数量(个)</div>
            <CountTo class="text-20px" :duration="2600" :end-val="recordSummary.successCount" :start-val="0" />
          </div>
        </div>
      </ContentWrap>
    </el-col>
      ...
      <ContentWrap class="h-[110px]">
        <div class="flex items-center">
          ...
          <div class="ml-[20px]">
            ...
            <CountTo class="text-20px" :duration="2600" :end-val="recordSummary.virtualGroupCount" :start-val="0" />
          </div>
        </div>
      </ContentWrap>
    </el-col>
    ...
    <el-form ref="queryFormRef" class="-mb-15px" :inline="true" label-width="68px" :model="queryParams">
      ...
      <el-form-item label="拼团状态" prop="status">
        <el-select v-model="queryParams.status" class="!w-240px" clearable placeholder="全部">
          <el-option
            v-for="(dict, index) in getIntDictOptions(DICT_TYPE.PROMOTION_COMBINATION_RECORD_STATUS)"
            :key="index"
            :label="dict.label"
            :value="dict.value"
          />
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="pageList">
      ...
      <el-table-column align="center" :formatter="dateFormatter" label="开团时间" prop="startTime" width="180" />
      <el-table-column align="center" label="拼团商品" min-width="300" prop="type" show-overflow-tooltip>
        <template #defaul="{ row }">
          <el-image class="mr-5px h-30px w-30px align-middle" :src="row.picUrl" @click="imagePreview(row.picUrl)" />
      ...
      <el-table-column align="center" fixed="right" label="操作">
        <template #default="scope">
          <el-button v-hasPermi="['promotion:combination-record:query']" link type="primary" @click="openRecordListDialog(scope.row)">
            查看拼团
          </el-button>
        </template>
      </el-table-column>
