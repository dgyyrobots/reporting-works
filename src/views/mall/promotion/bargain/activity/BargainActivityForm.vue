<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="65%">
    <Form ref="formRef" v-loading="formLoading" class="mt-10px" :is-col="true" :rules="rules" :schema="allSchemas.formSchema">
      <template #spuId>
        ...
        <SpuAndSkuList ref="spuAndSkuListRef" :rule-config="ruleConfig" :spu-list="spuList" :spu-property-list-p="spuPropertyList">
          <el-table-column align="center" label="砍价起始价格(元)" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.bargainFirstPrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
            </template>
          </el-table-column>
          <el-table-column align="center" label="砍价底价(元)" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.bargainMinPrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
            </template>
          </el-table-column>
...
import { RuleConfig, getPropertyList } from '/@/views/mall/product/spu/components'
