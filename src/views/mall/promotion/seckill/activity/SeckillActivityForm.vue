        ...
        <SpuAndSkuList ref="spuAndSkuListRef" :rule-config="ruleConfig" :spu-list="spuList" :spu-property-list-p="spuPropertyList">
          <el-table-column align="center" label="秒杀库存" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.stock" class="w-100%" :min="0" />
            </template>
          </el-table-column>
          <el-table-column align="center" label="秒杀价格(元)" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.seckillPrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
            </template>
          </el-table-column>
        </SpuAndSkuList>
      </template>
