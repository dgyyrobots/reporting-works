<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form ref="formRef" v-loading="formLoading" label-width="140px" :model="formData" :rules="formRules">
      ...
      <el-form-item label="优惠类型" prop="discountType">
        <el-radio-group v-model="formData.discountType">
          <el-radio v-for="dict in getIntDictOptions(DICT_TYPE.PROMOTION_DISCOUNT_TYPE)" :key="dict.value" :label="dict.value">
            {{ dict.label }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item v-if="formData.discountType === PromotionDiscountTypeEnum.PERCENT.type" label="最多优惠" prop="discountLimitPrice">
        <el-input-number v-model="formData.discountLimitPrice" class="mr-2 !w-400px" :min="0" placeholder="请输入最多优惠" :precision="2" />
        元
      </el-form-item>
      <el-form-item label="满多少元可以使用" prop="usePrice">
        <el-input-number v-model="formData.usePrice" class="mr-2 !w-400px" :min="0" placeholder="无门槛请设为 0" :precision="2" />
        元
      </el-form-item>
      ...
      <el-form-item v-if="formData.validityType === CouponTemplateValidityTypeEnum.TERM.type" label="领取日期" prop="fixedStartTerm">
        第
        <el-input-number v-model="formData.fixedStartTerm" class="mx-2" :min="0" placeholder="0 为今天生效" :precision="0" />
        至
        <el-input-number v-model="formData.fixedEndTerm" class="mx-2" :min="0" placeholder="请输入结束天数" :precision="0" />
        天有效
      </el-form-item>
    </el-form>
    <template #footer>
