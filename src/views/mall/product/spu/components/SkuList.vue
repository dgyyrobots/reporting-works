<template>
  ...
  >
    ...
    <template v-if="formData!.specType && !isBatch">
      <!--  根据商品属性动态添加 -->
      <el-table-column v-for="(item, index) in tableHeaders" :key="index" align="center" :label="item.label" min-width="120">
        <template #default="{ row }">
    ...
    <el-table-column align="center" label="销售价(元)" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.price" class="w-100%" :min="0" :precision="2" :step="0.1" />
      </template>
    </el-table-column>
    <el-table-column align="center" label="市场价(元)" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.marketPrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
      </template>
    </el-table-column>
    <el-table-column align="center" label="成本价(元)" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.costPrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
      </template>
    </el-table-column>
    <el-table-column align="center" label="库存" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.stock" class="w-100%" :min="0" />
      </template>
    </el-table-column>
    <el-table-column align="center" label="重量(kg)" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.weight" class="w-100%" :min="0" :precision="2" :step="0.1" />
      </template>
    </el-table-column>
    <el-table-column align="center" label="体积(m^3)" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.volume" class="w-100%" :min="0" :precision="2" :step="0.1" />
      </template>
    </el-table-column>
    <template v-if="formData!.subCommissionType">
      <el-table-column align="center" label="一级返佣(元)" min-width="168">
        <template #default="{ row }">
          <el-input-number v-model="row.firstBrokeragePrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
        </template>
      </el-table-column>
      <el-table-column align="center" label="二级返佣(元)" min-width="168">
        <template #default="{ row }">
          <el-input-number v-model="row.secondBrokeragePrice" class="w-100%" :min="0" :precision="2" :step="0.1" />
        </template>
      </el-table-column>
    </template>
  ...
  >
    ...
    <el-table-column align="center" label="图片" min-width="80">
      <template #default="{ row }">
        <el-image class="h-60px w-60px" :src="row.picUrl" @click="imagePreview(row.picUrl)" />
      </template>
    </el-table-column>
    <template v-if="formData!.specType && !isBatch">
      <!--  根据商品属性动态添加 -->
      <el-table-column v-for="(item, index) in tableHeaders" :key="index" align="center" :label="item.label" min-width="80">
        <template #default="{ row }">
          <span style="font-weight: bold; color: #40aaff">
            {{ row.properties[index]?.valueName }}
          </span>
  ...
  <!-- 情况三：作为活动组件 -->
  <el-table v-if="isActivityComponent" border :data="formData!.skus!" max-height="500" size="small" style="width: 99%">
    <el-table-column v-if="isComponent" type="selection" width="45" />
    <el-table-column align="center" label="图片" min-width="80">
      <template #default="{ row }">
        <el-image class="h-60px w-60px" :src="row.picUrl" @click="imagePreview(row.picUrl)" />
