  ...
  <el-table-column class-name="order-table-col">
    <template #header>
      <div class="flex items-center" style="width: 100%">
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[0] + 'px' }">商品信息</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[1] + 'px' }">单价(元)/数量</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[2] + 'px' }">售后状态</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[3] + 'px' }">实付金额(元)</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[4] + 'px' }">买家/收货人</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[5] + 'px' }">配送方式</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[6] + 'px' }">订单状态</div>
        <div class="flex justify-center" :style="{ width: orderTableHeadWidthList[7] + 'px' }">操作</div>
      </div>
    </template>
    <template #default="scope">
      ...
      >
        <el-table-column min-width="300" prop="spuName">
          <template #header>
            <div class="mr-[20px] h-[35px] flex items-center pl-[10px] pr-[10px]" style="background-color: #f7f7f7">
              <span class="mr-20px">订单号：{{ scope.row.no }}</span>
              <span class="mr-20px">下单时间：{{ formatDate(scope.row.createTime) }}</span>
              <span>订单来源：</span>
              <dict-tag class="mr-20px" :type="DICT_TYPE.TERMINAL" :value="scope.row.terminal" />
              <span>支付方式：</span>
              <dict-tag
          ...
          <template #default="{ row }">
            <div class="flex flex-wrap">
              <div class="mb-[10px] mr-[10px] flex items-start">
                <div class="mr-[10px]">
                  <el-image class="!h-[45px] !w-[45px]" fit="contain" :src="row.picUrl" @click="imagePreview(row.picUrl)">
                    <template #error>
              ...
              <el-tag v-for="property in row.properties" :key="property.propertyId" class="mb-[10px] mr-[10px]">
        ...
        <el-table-column label="买家/收货人" min-width="160">
          <template #default>
            ...
            <div v-if="scope.row.deliveryType === DeliveryTypeEnum.EXPRESS.type" class="flex flex-col">
              ...
              <span>
                收货人：{{ scope.row.receiverName }} {{ scope.row.receiverMobile }} {{ scope.row.receiverAreaName }}
