  ...
  <ContentWrap>
    ...
    <el-descriptions :column="1" title="订单状态">
      ...
      <el-descriptions-item v-hasPermi="['trade:order:update']" label-class-name="no-colon">
        ...
        <template v-if="formData.status! === TradeOrderStatusEnum.UNDELIVERED.status">
          ...
          <el-button v-if="formData.deliveryType === DeliveryTypeEnum.EXPRESS.type" type="primary" @click="updateAddress">
            修改地址
          </el-button>
          <!-- 到店自提 -->
          <el-button v-if="formData.deliveryType === DeliveryTypeEnum.PICK_UP.type && showPickUp" type="primary" @click="handlePickUp">
            核销
          </el-button>
        </template>
      </el-descriptions-item>
    ...
    <el-descriptions title="商品信息">
      <el-descriptions-item label-class-name="no-colon">
        <el-row :gutter="20">
          <el-col :span="15">
            <el-table border :data="formData.items">
    ...
    <el-descriptions title="订单操作日志">
      <el-descriptions-item label-class-name="no-colon">
        <el-timeline>
          <el-timeline-item v-for="(log, index) in formData.logs" :key="index" placement="top" :timestamp="formatDate(log.createTime!)">
            <div class="el-timeline-right-content">
              {{ log.content }}
            </div>
            <template #dot>
              <span class="dot-node-style" :style="{ backgroundColor: getUserTypeColor(log.userType!) }">
