<template>
  ...
  <ContentWrap>
    <el-form ref="queryFormRef" class="-mb-15px" :inline="true" label-width="68px" :model="queryParams">
      ...
      <el-form-item>
        ...
        <el-button v-hasPermi="['trade:order:pick-up']" plain type="success" @click="handlePickup">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="推荐人信息" min-width="100" prop="brokerageUser.nickname" />
      <el-table-column align="center" label="商品信息" min-width="300" prop="spuName">
        <template #default="{ row }">
          <div v-for="item in row.items" :key="item.id" class="flex items-center">
            <el-image class="mr-10px h-30px w-30px flex-shrink-0" :preview-src-list="[item.picUrl]" preview-teleported :src="item.picUrl" />
      ...
      <el-table-column align="center" :formatter="fenToYuanFormat" label="实付金额(元)" min-width="110" prop="payPrice" />
      ...
      <el-table-column align="center" :formatter="dateFormatter" label="下单时间" min-width="170" prop="createTime" />
    </el-table>
    <!-- 分页 -->
...
import * as PickUpStoreApi from '/@/api/mall/trade/delivery/pickUpStore/index'
import { DeliveryPickUpStoreVO } from '/@/api/mall/trade/delivery/pickUpStore/index'
import * as TradeOrderApi from '/@/api/mall/trade/order/index'
import { TradeOrderSummaryRespVO } from '/@/api/mall/trade/order/index'
...
const inputChangeSelect = (val: string) => {
  dynamicSearchList.value
    ...
    ?.forEach((item) => {
      // 清除集合搜索无用属性
      if (item.value in queryParams.value) {
