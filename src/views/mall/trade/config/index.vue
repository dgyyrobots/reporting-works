        ...
        <el-tab-pane label="分销">
          ...
          <el-form-item label="分佣模式" prop="brokerageEnabledCondition">
            <el-radio-group v-model="formData.brokerageEnabledCondition">
              <el-radio v-for="dict in getIntDictOptions(DICT_TYPE.BROKERAGE_ENABLED_CONDITION)" :key="dict.value" :label="dict.value">
                {{ dict.label }}
              </el-radio>
            </el-radio-group>
          ...
          <el-form-item label="分销关系绑定" prop="brokerageBindMode">
            <el-radio-group v-model="formData.brokerageBindMode">
              <el-radio v-for="dict in getIntDictOptions(DICT_TYPE.BROKERAGE_BIND_MODE)" :key="dict.value" :label="dict.value">
                {{ dict.label }}
              </el-radio>
            </el-radio-group>
          ...
          <el-form-item label="一级返佣比例" prop="brokerageFirstPercent">
            <el-input-number v-model="formData.brokerageFirstPercent" class="!w-xs" :max="100" :min="0" placeholder="请输入一级返佣比例" />
            <el-text class="w-full" size="small" type="info">订单交易成功后给推广人返佣的百分比</el-text>
          </el-form-item>
          <el-form-item label="二级返佣比例" prop="brokerageSecondPercent">
            <el-input-number v-model="formData.brokerageSecondPercent" class="!w-xs" :max="100" :min="0" placeholder="请输入二级返佣比例" />
          ...
          <el-form-item label="佣金冻结天数" prop="brokerageFrozenDays">
            <el-input-number v-model="formData.brokerageFrozenDays" class="!w-xs" :min="0" placeholder="请输入佣金冻结天数" />
          ...
          <el-form-item label="提现方式" prop="brokerageWithdrawTypes">
            <el-checkbox-group v-model="formData.brokerageWithdrawTypes">
              <el-checkbox v-for="dict in getIntDictOptions(DICT_TYPE.BROKERAGE_WITHDRAW_TYPE)" :key="dict.value" :label="dict.value">
                {{ dict.label }}
              </el-checkbox>
            </el-checkbox-group>
          ...
          <el-form-item label="提现银行" prop="brokerageBankNames">
            <el-select v-model="formData.brokerageBankNames" multiple placeholder="请选择提现银行">
              <el-option
