  ...
  <Dialog v-model="dialogVisible" title="订单详情" width="700px">
    <el-descriptions :column="2" label-class-name="desc-label">
      ...
      <el-descriptions-item label="支付状态">
        <dict-tag size="small" :type="DICT_TYPE.PAY_ORDER_STATUS" :value="detailData.status" />
      </el-descriptions-item>
      <el-descriptions-item label="支付金额">
        <el-tag size="small" type="success">￥{{ (detailData.price / 100.0).toFixed(2) }}</el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="手续费">
        <el-tag size="small" type="warning">￥{{ (detailData.channelFeePrice / 100.0).toFixed(2) }}</el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="手续费比例">{{ (detailData.channelFeeRate / 100.0).toFixed(2) }}%</el-descriptions-item>
    ...
    <el-descriptions :column="2" label-class-name="desc-label">
      ...
      <el-descriptions-item label="退款金额">
        <el-tag size="mini" type="danger">￥{{ (detailData.refundPrice / 100.0).toFixed(2) }}</el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="通知 URL">{{ detailData.notifyUrl }}</el-descriptions-item>
    </el-descriptions>
    <!-- 分割线 -->
    <el-divider />
    <el-descriptions border :column="1" direction="vertical" label-class-name="desc-label">
