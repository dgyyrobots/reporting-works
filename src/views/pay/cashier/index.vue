<template>
  ...
  <el-card v-loading="loading">
    <el-descriptions border :column="3" title="支付信息">
  ...
  <el-card v-loading="submitLoading" element-loading-text="提交支付中..." style="margin-top: 10px">
    ...
    <div class="pay-channel-container">
      <div v-for="channel in channelsAlipay" :key="channel.code" class="box" @click="submit(channel.code)">
    ...
    <div class="pay-channel-container">
      <div v-for="channel in channelsWechat" :key="channel.code" class="box" @click="submit(channel.code)">
        <img :src="channel.icon" />
        <div class="title">{{ channel.name }}</div>
      </div>
    </div>
    ...
    <div class="pay-channel-container">
      <div v-for="channel in channelsMock" :key="channel.code" class="box" @click="submit(channel.code)">
        <img :src="channel.icon" />
        <div class="title">{{ channel.name }}</div>
      </div>
    </div>
  ...
  <!-- 展示形式：二维码 URL -->
  <Dialog v-model="qrCode.visible" append-to-body :close-on-press-escape="false" :title="qrCode.title" width="350px">
    <Qrcode :text="qrCode.url" :width="310" />
  </Dialog>

  <!-- 展示形式：BarCode 条形码 -->
  <Dialog v-model="barCode.visible" append-to-body :close-on-press-escape="false" :title="barCode.title" width="500px">
      ...
      <el-row>
        ...
        <el-col :span="24">
          <div style="text-align: right">
            或使用
            <el-link href="https://baike.baidu.com/item/条码支付/10711903" target="_blank" type="danger">(扫码枪/扫码盒)</el-link>
            扫码
          </div>
        </el-col>
      </el-row>
    ...
    <template #footer>
      <el-button :disabled="barCode.value.length === 0" type="primary" @click="submit0(barCode.channelCode)">确认支付</el-button>
