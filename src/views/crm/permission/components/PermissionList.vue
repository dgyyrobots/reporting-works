<template>
  ...
  <ContentWrap class="mt-10px">
    <el-table v-loading="loading" :data="list" :show-overflow-tooltip="true" :stripe="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" :formatter="dateFormatter" label="加入时间" prop="createTime" />
    </el-table>
  </ContentWrap>
...
import * as PermissionApi from '/@/api/crm/permission/index'
import { PermissionLevelEnum } from '/@/api/crm/permission/index'
...
const handleQuit = async () => {
  const permission = list.value.find((item) => item.userId === userStore.getUser.id && item.level === PermissionLevelEnum.OWNER)
  if (permission) {
