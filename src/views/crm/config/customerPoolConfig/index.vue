<template>
  <ContentWrap>
    <el-form ref="formRef" v-loading="formLoading" label-width="160px" :model="formData" :rules="formRules">
      <el-card shadow="never">
        <!-- 操作 -->
        <template #header>
          <div class="flex items-center justify-between">
            <CardTitle title="客户公海规则设置" />
            <el-button v-hasPermi="['crm:customer-pool-config:update']" type="primary" @click="onSubmit">保存</el-button>
          </div>
        </template>
        <!-- 表单 -->
        ...
        <div v-if="formData.enabled">
          ...
          <el-form-item label="提前提醒设置" prop="notifyEnabled">
            <el-radio-group v-model="formData.notifyEnabled" class="ml-4" @change="changeNotifyEnable">
          ...
          <div v-if="formData.notifyEnabled">
            <el-form-item>
              提前
              <el-input-number v-model="formData.notifyDays" class="mx-2" />
              天提醒
            </el-form-item>
          </div>
        </div>
