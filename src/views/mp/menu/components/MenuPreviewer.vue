<template>
  <draggable v-model="menuList" :animation="400" ghost-class="draggable-ghost" item-key="id" @end="onParentDragEnd">
    <template #item="{ element: parent, index: x }">
      <div class="menu_bottom">
        <!-- 一级菜单 -->
        <div class="menu_item" :class="{ active: props.activeIndex === `${x}` }" @click="menuClicked(parent, x)">
          <Icon color="black" icon="ep:fold" />
          {{ parent.name }}
        </div>
        <!-- 以下为二级菜单-->
        <div v-if="props.parentIndex === x && parent.children" class="submenu">
          <draggable v-model="parent.children" :animation="400" ghost-class="draggable-ghost" item-key="id" @end="onChildDragEnd">
          ...
          <div v-if="!parent.children || parent.children.length < 5" class="menu_bottom menu_addicon" @click="addSubMenu(x, parent)">
