<template>
  <div>
    ...
    <el-row v-else style="text-align: center">
      <!-- 选择素材 -->
      <el-col class="col-select" :span="12">
        <el-button type="success" @click="showDialog = true">
          素材库选择
          <Icon icon="ep:circle-check" />
        </el-button>
        <el-dialog v-model="showDialog" append-to-body destroy-on-close title="选择语音" width="90%">
          <WxMaterialSelect :account-id="reply.accountId" type="voice" @select-material="selectMaterial" />
        </el-dialog>
      </el-col>
      <!-- 文件上传 -->
      <el-col class="col-add" :span="12">
        <el-upload
          :action="UPLOAD_URL"
