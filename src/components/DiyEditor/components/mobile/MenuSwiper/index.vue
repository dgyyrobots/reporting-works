<template>
  <el-carousel arrow="hover" :autoplay="false" :height="`${carouselHeight}px`" indicator-position="outside">
      ...
      <div class="flex flex-row flex-wrap">
        ...
        >
          ...
          <div class="relative" :class="`h-${ICON_SIZE}px w-${ICON_SIZE}px`">
            ...
            <el-image v-if="item.iconUrl" class="h-full w-full" :src="item.iconUrl" />
          </div>
          <!-- 标题 -->
          <span
...
watch(
  ...
  () => {
    ...
    rowHeight.value = (props.property.layout === 'iconText' ? ICON_SIZE + TITLE_HEIGHT : ICON_SIZE) + SPACE_Y * 2
    // 计算轮播的高度：行数 * 行高
