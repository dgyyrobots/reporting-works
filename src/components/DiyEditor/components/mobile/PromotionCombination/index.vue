  ...
  <el-scrollbar ref="containerRef" class="z-1 min-h-30px" wrap-class="w-full">
    ...
    >
      ...
      >
        ...
        >
          <!-- 商品名称 -->
          <div v-if="property.fields.name.show" class="truncate text-12px" :style="{ color: property.fields.name.color }">
            {{ spu.name }}
          </div>
          <div>
            <!-- 商品价格 -->
            <span v-if="property.fields.price.show" class="text-12px" :style="{ color: property.fields.price.color }">
              ￥{{ spu.price }}
            </span>
          </div>
        </div>
