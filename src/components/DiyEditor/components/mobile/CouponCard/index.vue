  ...
  <el-scrollbar ref="containerRef" class="z-1 min-h-30px" wrap-class="w-full">
    ...
    >
      <div
        ...
        class="box-content"
        :style="{
          background: property.bgImg ? `url(${property.bgImg}) 100% center / 100% 100% no-repeat` : '#fff',
      ...
      >
        ...
        <div v-if="property.columns === 1" class="m-l-16px flex flex-row justify-between p-8px">
          <div class="flex flex-col justify-evenly gap-4px">
            ...
            <!-- 优惠描述 -->
            <coupon-discount-desc :coupon="coupon" />
            <!-- 有效期 -->
            <coupon-valid-term :coupon="coupon" />
          </div>
        ...
        <!-- 布局2：2列-->
        <div v-else-if="property.columns === 2" class="m-l-16px flex flex-row justify-between p-8px">
          <div class="flex flex-col justify-evenly gap-4px">
            <!-- 优惠值 -->
            <coupon-discount :coupon="coupon" />
            <div>{{ coupon.name }}</div>
...
import { CouponDiscountDesc, CouponValidTerm } from '/@/components/DiyEditor/components/mobile/CouponCard/component'
...
watch(
  ...
  () => {
    ...
    couponWidth.value = (phoneWidth.value * 0.95 - props.property.space * (props.property.columns - 1)) / props.property.columns
    // 显示滚动条
    scrollbarWidth.value = `${couponWidth.value * couponList.value.length + props.property.space * (couponList.value.length - 1)}px`
  },
