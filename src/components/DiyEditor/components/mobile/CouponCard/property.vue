<template>
  <component-container-property v-model="formData.style">
    <el-form label-width="80px" :model="formData">
      <el-card class="property-group" header="优惠券列表" shadow="never">
        <div v-for="(coupon, index) in couponList" :key="index" class="flex items-center justify-between">
          ...
          <el-text truncated type="info">
            ...
            <span v-if="coupon.discountType === PromotionDiscountTypeEnum.PRICE.type">减{{ floatToFixed2(coupon.discountPrice) }}元</span>
  ...
  <!-- 优惠券选择 -->
  <coupon-select ref="couponSelectDialog" v-model:multiple-selection="couponList" />
</template>
