      ...
      <el-card class="property-group" header="组件样式">
        <el-form label-width="80px" :model="formData">
          ...
          <el-form-item v-if="formData.bgType === 'color'" label="选择颜色" prop="bgColor">
            <color-input v-model="formData.bgColor" />
          </el-form-item>
          <el-form-item v-else label="上传图片" prop="bgImg">
            <upload-img v-model="formData.bgImg" :limit="1">
              <template #tip>建议宽度 750px</template>
            </upload-img>
          </el-form-item>
          <el-tree :data="treeData" :expand-on-click-node="false">
            <template #default="{ node, data }">
              <el-form-item class="w-full m-b-0!" :label="data.label" :label-width="node.level === 1 ? '80px' : '62px'" :prop="data.prop">
                <el-slider
          ...
          <slot :form-data="formData" name="style"></slot>
        </el-form>
      </el-card>
    </el-tab-pane>
  </el-tabs>
