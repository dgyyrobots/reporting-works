<template>
    ...
    <div class="h-500px flex gap-8px">
      ...
      <el-scrollbar ref="groupScrollbar" view-class="flex flex-col" wrap-class="h-full">
        <el-button
          ...
          :key="groupIndex"
          ref="groupBtnRefs"
          :class="['m-r-16px m-l-0px! justify-start! w-90px', { active: activeGroup === group.name }]"
      ...
      <el-scrollbar ref="linkScrollbar" class="h-full flex-1" @scroll="handleScroll">
        <div v-for="(group, groupIndex) in APP_LINK_GROUP_LIST" :key="groupIndex">
          ...
          <!-- 链接列表 -->
          <el-tooltip v-for="(appLink, appLinkIndex) in group.links" :key="appLinkIndex" :content="appLink.path" placement="bottom">
            <el-button
  ...
  <Dialog v-model="detailSelectDialog.visible" title="" width="50%">
    <el-form class="min-h-200px">
      <el-form-item v-if="detailSelectDialog.type === APP_LINK_TYPE_ENUM.PRODUCT_CATEGORY_LIST" label="选择分类">
        <product-category-select v-model="detailSelectDialog.id" :parent-id="0" @update:model-value="handleProductCategorySelected" />
      </el-form-item>
    </el-form>
  </Dialog>
</template>
...
import { getUrlNumberValue } from '/@/utils/index.ts'
...
const open = (link: string) => {
  ...
  // 滚动到当前的链接
  const group = APP_LINK_GROUP_LIST.find((group) => group.links.some((linkItem) => isSameLink(linkItem.path, link)))
  if (group) {
...
const handleAppLinkSelected = (appLink: any) => {
  ...
  switch (appLink.type) {
    case APP_LINK_TYPE_ENUM.PRODUCT_CATEGORY_LIST:
      ...
      // 返显
      detailSelectDialog.value.id = getUrlNumberValue('id', `http://127.0.0.1${activeAppLink.value}`) || undefined
      break
    default:
      break
  }
...
const scrollToGroupBtn = (group: string) => {
  const groupBtn = groupBtnRefs.value.map((btn) => btn['ref']).find((ref) => ref.textContent === group)
  if (groupBtn) {
