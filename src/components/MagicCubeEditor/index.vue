<template>
  <div class="relative">
    <table class="cube-table">
      ...
      >
        <!-- 右上角热区删除按钮 -->
        <div v-if="selectedHotAreaIndex === index" class="btn-delete" @click="handleDeleteHotArea(index)">
...
import { Point, Rect, createRect, isContains, isOverlap } from './util'
...
const handleCellHover = (currentRow: number, currentCol: number) => {
  ...
  // 当前已选的区域
  const currentSelectedArea = createRect(hotAreaBeginCube.value!, cubes.value[currentRow][currentCol])
  // 热区不允许重叠
