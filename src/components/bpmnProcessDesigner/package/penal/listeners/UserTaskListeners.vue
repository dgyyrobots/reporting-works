<template>
  <div class="panel-tab__content">
    <el-table border :data="elementListenersList" size="small">
      <el-table-column label="序号" type="index" width="50px" />
      <el-table-column :formatter="(row) => listenerEventTypeObject[row.event]" label="事件类型" min-width="80px" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="90px">
        <template #default="scope">
          <el-button link size="small" @click="openListenerForm(scope.row, scope.$index)">编辑</el-button>
          <el-divider direction="vertical" />
          <el-button link size="small" style="color: #ff4d4f" @click="removeListener(scope.row, scope.$index)">移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="element-drawer__button">
      <x-button pre-icon="ep:plus" size="small" title="添加监听器" type="primary" @click="openListenerForm(null)" />
    </div>

    <!-- 监听器 编辑/创建 部分 -->
    <el-drawer v-model="listenerFormModelVisible" append-to-body destroy-on-close :size="`${width}px`" title="任务监听器">
      <el-form ref="listenerFormRef" label-width="96px" :model="listenerForm" size="small">
      ...
      <el-table border :data="fieldsListOfListener" fit max-height="240" size="small" style="flex: none">
        <el-table-column label="序号" type="index" width="50px" />
        <el-table-column label="字段名称" min-width="100px" prop="name" />
        <el-table-column :formatter="(row) => fieldTypeObject[row.fieldType]" label="字段类型" min-width="80px" show-overflow-tooltip />
        <el-table-column :formatter="(row) => row.string || row.expression" label="字段值/表达式" min-width="100px" show-overflow-tooltip />
        <el-table-column label="操作" width="100px">
          <template #default="scope">
            <el-button link size="small" @click="openListenerFieldForm(scope.row, scope.$index)">编辑</el-button>
            <el-divider direction="vertical" />
            <el-button link size="small" style="color: #ff4d4f" @click="removeListenerField(scope.row, scope.$index)">移除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="listenerFieldFormModelVisible" append-to-body destroy-on-close title="字段配置" width="600px">
      <el-form ref="listenerFieldFormRef" label-width="96px" :model="listenerFieldForm" size="small" style="height: 136px">
