    ...
    <el-form-item label="消息实例">
      <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: nowrap">
        <el-select v-model="bindMessageId" @change="updateTaskMessage">
          <el-option v-for="key in Object.keys(messageMap)" :key="key" :label="messageMap[key]" :value="key" />
        </el-select>
        <x-button pre-icon="ep:plus" style="margin-left: 8px" type="primary" @click="openMessageModel" />
      </div>
    </el-form-item>
    <el-dialog v-model="messageModelVisible" append-to-body :close-on-click-modal="false" destroy-on-close title="创建新消息" width="400px">
      <el-form label-width="90px" :model="newMessageForm" size="small">
