<template>
  <div class="flex flex-row items-center gap-3 rounded bg-[var(--el-bg-color-overlay)] p-4">
    <div class="h-12 w-12 flex flex-shrink-0 items-center justify-center rounded-1" :class="`${iconColor} ${iconBgColor}`">
      <icon class="!text-6" :icon="icon" />
    </div>
    <div class="flex flex-col gap-1">
      ...
      <div class="flex flex-row items-baseline gap-2">
        <div class="text-7">
          <count-to :decimals="decimals" :end-val="value" :prefix="prefix" />
        </div>
        <span v-if="percent != undefined" :class="toNumber(percent) > 0 ? 'text-red-500' : 'text-green-500'">
          <span class="text-sm">{{ Math.abs(toNumber(percent)) }}%</span>
          <icon class="ml-0.5 !text-3" :icon="toNumber(percent) > 0 ? 'ep:caret-top' : 'ep:caret-bottom'" />
        </span>
      </div>
    </div>
  </div>
