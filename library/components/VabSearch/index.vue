...
import { generateRoutePath } from '/@/utils/routerHelper'
...
const addFieldToTree = (routes: any, parent = '') => {
  routes.forEach((node: any) => {
    node.value = node.name
    node.fullPath = generateRoutePath(parent, node.path)
    node.label = translate(node.meta.title)
    if (node.children && node.children.length) addFieldToTree(node.children, node.fullPath)
  })
  return routes
}
  ...
  nextTick(() => {
    if (!item.children)
      if (isExternal(item.path)) {
        ...
        router.push('/redirect')
        return
      } else router.push(item.fullPath)
  })
}
