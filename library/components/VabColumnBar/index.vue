<template>
  ...
  >
    ...
    <el-tabs v-model="tab.data" tab-position="left" @tab-click="handleTabClick">
      <template v-for="(item, index) in routes" :key="index">
        <template v-if="item.children?.length == 1">
          <el-tab-pane :name="item.children[0].name">
            <template #label>
              <div
                ...
                :title="translate(item.children[0].meta.title)"
              >
                <div>
                  <!-- <vab-icon
                    v-if="item.children[0].meta.icon"
                    :icon="item.children[0].meta.icon"
                    :is-custom-svg="item.children[0].meta.isCustomSvg"
                  /> -->
                  <icon  :icon="item.children[0].meta.icon"
                    :is-custom-svg="item.children[0].meta.isCustomSvg" />
                  <span v-if="translate(item.children[0].meta.title).length < 4">
                    {{ translate(item.children[0].meta.title) }}
                  </span>
          ...
          <!-- <el-button @click="console.log(JSON.parse(JSON.stringify(item)))" type="primary">1</el-button> -->
        </template>
        <template v-else >
          ...
          <template #label>
            ...
            >
              <div>
                <!-- <vab-icon v-if="item.meta.icon"
                :icon="item.meta.icon" :is-custom-svg="item.meta.isCustomSvg" /> -->
                <icon  :icon="item.meta.icon" :is-custom-svg="item.meta.isCustomSvg" />
          ...
          <!-- <el-button @click="console.log(item)" type="primary">
          <vab-icon icon = 'home-2-line'>1</vab-icon>
          </el-button>
          <el-button @click="console.log(item)" type="primary">
    ...
    >
      <vab-menu v-for="item in partialRoutes" :key="item.path" :item="item" :parent="tabMenu.path" />
    </el-menu>
...
const handleTabClick = () => {
  nextTick(() => {
    if (tabMenu.value.meta.target === '_blank') {
      ...
      router.push('/redirect')
    } else if (openFirstMenu) router.push(getRedirect(tabMenu.value.path, tabMenu.value.children))
    setDefaultOpeneds()
  })
}
